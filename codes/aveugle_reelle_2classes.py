...
def calc_probapost_Gauss(Y, p1, p2, m1, sig1, m2, sig2):
    ...
    total[total == 0] = np.finfo(float).eps  # Remplacer les valeurs nulles par un epsilon très petit
...
if __name__ == "__main__":
    ...
    p10, p20, m10, sig10, m20, sig20 = 0.5, 0.5, 1, 1, 4, 1  # Paramètres pour SEM basés sur les meilleures performances

    # Estimation des paramètres avec SEM
    p1_sem, p2_sem, m1_sem, sig1_sem, m2_sem, sig2_sem = calc_SEM(Y, p10, p20, m10, sig10, m20, sig20, nb_iter=10)
    ...
    affiche_image('Image Segmentée', X_seg, save_path=os.path.join(output_folder, 'image_segmentee_reelle.png'))
    
    # Sauvegarder également l'image segmentée
    plt.imsave(os.path.join(output_folder, 'image_segmentee_reelle_saved.png'), X_seg, cmap='gray')
